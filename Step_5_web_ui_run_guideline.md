...
- `web_app.py` is a FastAPI server that exposes the RAG pipeline over HTTP and serves two pages:
...
| Error | Fix |
|---|---|
| `uvicorn: command not found` | Run `python -m uvicorn web_app:app --port 8000` instead |
| Port 8000 already in use | Use another port, e.g. `uvicorn web_app:app --port 8001` |
